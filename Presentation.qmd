...
2. OSC-Specific Setup (5 min)
3. Understanding nf-core/rnaseq (5 min)
4. **Hands-On Exercise** (15 min)
5. Customization (5 min)
6. Troubleshooting (5 min)
7. Reproducibility Best Practices (3 min)
8. Advanced Topics (2 min)
...
or with `setup_nextflow.sh`

## Creating Nextflow Configuration for OSC
...
# Section 3: Understanding nf-core/rnaseq
...
# Section 4: Reference Genomes - iGenomes
...
**Script location:** `./run_rnaseq.sh` (ready to use)
...
# Section 6: Customization and Configuration
...
## Common Issue 2: Pipeline Hangs/Stalls

**Diagnosis:**

```bash
# Check if jobs are queued
squeue -u $USER -o "%.18i %.9P %.130j %.8T %.10M"

# Check Nextflow log
tail -f .nextflow.log

# Look for waiting processes
grep "Submitted process" .nextflow.log | tail -20
```

**Solution:** Check queue limits in `osc.config`

## Common Issue 3: Container Download Failures
...
# Note: -concretize works in Nextflow 23.10.x
...
# Section 8: Reproducibility Best Practices
...
## Date: 2026-01-28
## Pipeline: nf-core/rnaseq v3.22.2
## Nextflow: v24.10.5
## Reference Genome: GRCh38 (iGenomes / Ensembl 113)
## Command:
nextflow run nf-core/rnaseq -r 3.22.2 --genome GRCh38 ...
```
...
- **OSC Help**: [oschelp@osc.edu](mailto:oschelp@osc.edu)
